...
with tab_summary:
    ...
    # Apply bold styling using a function
    def highlight_account(val):
        if isinstance(val, str) and "GTS" in val:
            return "font-weight: bold; background-color: #f0f0f0"
        elif isinstance(val, str) and "NTS" in val:
            return "font-weight: bold; background-color: #f0f0f0"
        elif isinstance(val, str) and "GP" in val:
            return "font-weight: bold; background-color: #e6f3e6"
        return ""
    ...
    st.dataframe(
        df_pl.style.format({
            "Value": "£{:,.0f}",
            "% of GTS": "{:.1f}%"
        }).applymap(highlight_account, subset=["Account"]),
